...
// main plugin class
class JobSearch_CareerJet_Jobs_Hooks {

    // hook things up
    public function __construct() {

        // job meta fields
        //add_action('jobsearch_job_meta_ext_fields', array($this, 'careerjet_job_meta_fields'));

        //
        add_action('admin_menu', array($this, 'jobsearch_careerjet_jobs_import_page'));
        add_action('wp_ajax_jobsearch_import_careerjet_jobs', array($this, 'jobsearch_import_careerjet_jobs'));

        // job listings shortcode params hook
        add_filter('jobsearch_job_integrations_types_list', array($this, 'shortcode_params_add'), 10, 1);

        // job listings query args params hook
        add_filter('jobsearch_job_listing_query_args_array', array($this, 'careerjet_jobs_listing_parameters'), 10, 2);
    }

    public function jobsearch_careerjet_jobs_import_page() {
        $careerjet_jobs_switch = get_option('jobsearch_integration_careerjet_jobs');
        if ($careerjet_jobs_switch == 'on') {
            add_submenu_page('edit.php?post_type=job', esc_html__('Import CareerJet Jobs', 'wp-jobsearch'), esc_html__('Import CareerJet Jobs', 'wp-jobsearch'), 'manage_options', 'import-careerjet-jobs', array($this, 'jobsearch_import_careerjet_jobs_settings'));
        }
    }

    /**
     * CareerJet jobs settings page
     * */
    public function jobsearch_import_careerjet_jobs_settings() {
        global $jobsearch_form_fields;
        
        
        $publisher_number = get_option('jobsearch_careerjet_imp_publisher_number');
        ?>
        <div id="wrapper" class="jobsearch-post-settings jobsearch-careerjet-import-sec">
            <h2><?php esc_html_e('Import CareerJet Jobs', 'wp-jobsearch'); ?></h2>
            <div class="error" id="error_msg"><p><strong><?php _e('There is some error importing jobs.', 'wp-jobsearch'); ?></strong></p></div>
            <div id="success_msg" class="updated"><p><strong><?php _e('CareerJet jobs are imported successfully.', 'wp-jobsearch'); ?></strong></p></div>
            
            <form id="jobsearch-import-careerjet-jobs" class="jobsearch-careerjet-jobs" method="post" enctype="multipart/form-data">
                <?php
                wp_nonce_field('jobsearch-import-careerjet-jobs-page', '_wpnonce-jobsearch-import-careerjet-jobs-page');
                ?>
                <div class="jobsearch-element-field">
                    ...
                    <div class="elem-field">
                        ...
                        $field_params = array(
                            ...
                            'id' => 'keywords',
                            'cus_name' => 'keywords',
                            'field_desc' => esc_html__('Enter job title, keywords or company name.', 'wp-jobsearch'),
                        );
                ...
                <div class="jobsearch-element-field">
                    <div class="elem-label">
                        <label><?php esc_html_e('Page', 'wp-jobsearch') ?></label>
                    </div>
                    <div class="elem-field">
                        <?php
                        $field_params = array(
                            ...
                            'id' => 'page',
                            'cus_name' => 'page',
                            'field_desc' => esc_html__('Enter page number.', 'wp-jobsearch'),
                        );
                        $jobsearch_form_fields->input_field($field_params);
                        ?>
                    </div>
                ...
                <div class="jobsearch-element-field">
                    ...
                    <div class="elem-field">
                        <input type="button" id="import-careerjet-jobs" class="import-careerjet-jobs" name="import-careerjet-jobs" onclick="javascript:jobsearch_import_careerjet_jobs_submit('<?php echo esc_js(admin_url('admin-ajax.php')) ?>');" value="<?php esc_html_e('Import CareerJet Jobs', 'wp-jobsearch') ?>">
    ...
    public function careerjet_job_meta_fields() {
        global $jobsearch_form_fields;
        ...
        if ($careerjet_jobs_switch == 'on') {
            ...
            <div class="jobsearch-elem-heading">
                <h2><?php esc_html_e('CareerJet Job Fields', 'wp-jobsearch') ?></h2>
            </div>
    ...
    public function jobsearch_import_careerjet_jobs() {
        $search_keywords = sanitize_text_field(stripslashes($_POST['keywords']));
        $search_location = sanitize_text_field(stripslashes($_POST['location']));
        $page_num = sanitize_text_field($_POST['page']);
        ...
        $careerjet_jobs = JobSearch_CareerJet_API::get_jobs($api_args);
        
        $json = array();
        if (isset($careerjet_jobs['error']) && $careerjet_jobs['error'] != '') {
            $json['type'] = 'error';
            $json['message'] = $careerjet_jobs['error'];
        } elseif (empty($careerjet_jobs) || $careerjet_jobs === NULL) {
            $json['type'] = 'error';
            ...
            foreach ($careerjet_jobs as $careerjet_job) {
                $job_title = isset($careerjet_job->title) ? $careerjet_job->title : '';
                $job_desc = isset($careerjet_job->description) ? $careerjet_job->description : '';
                $job_location = isset($careerjet_job->locations) ? $careerjet_job->locations : '';
                $job_url = isset($careerjet_job->url) ? $careerjet_job->url : '';
                $job_company = isset($careerjet_job->company) ? $careerjet_job->company : '';
                $job_salary_min = isset($careerjet_job->salary_min) ? $careerjet_job->salary_min : '';
                ...
                if ($existing_id > 0) {
                    ...
                    update_post_meta($post_id, 'jobsearch_job_referral', 'careerjet', true);
            ...
            if ($no_unique_jobs_found) {
                ...
                $json['type'] = 'success';
                $json['msg'] = sprintf(__('%s careerjet jobs are imported successfully.', 'wp-jobsearch'), count($careerjet_jobs));
            }
        }
        
        if (isset($_POST['action']) && $_POST['action'] == 'jobsearch_import_careerjet_jobs') {
    ...
    public function careerjet_jobs_listing_parameters($args, $attr) {

        $careerjet_jobs_switch = get_option('jobsearch_integration_careerjet_jobs');

        if ($careerjet_jobs_switch == 'on') {
            if (isset($attr['job_list_type']) && $attr['job_list_type'] == 'careerjet') {
                $filter_arr = array(
...
$JobSearch_CareerJet_Jobs_Hooks_obj = new JobSearch_CareerJet_Jobs_Hooks();
